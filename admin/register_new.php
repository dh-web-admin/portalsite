    ...
    if (empty($error)) {
        ...
        if (empty($error)) {
            if ($non_user) {
                ...
                // Provide an explicit empty string for password (rather than NULL)
                // so strict NOT NULL columns accept the insert.
                $sql = "INSERT INTO users (name, role, password) VALUES (?, ?, '')";
