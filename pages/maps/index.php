          ...
          <div style="margin-bottom: 12px; margin-top: 12px; display: flex; gap: 14px; align-items: flex-start; flex-shrink: 0;">
            ...
            <div style="margin-left: auto; position: relative;">
              <input 
                ...
                style="padding: 5px 10px; border: 1px solid #cbd5e1; border-radius: 4px; font-size: 11px; color: #334155; background: #fff; min-width: 180px; cursor: text;"
              />
              <div id="supplierList" style="display: none; position: absolute; top: calc(100% + 1px); left: 0; width: 100%; background: #fff; border: 1px solid #cbd5e1; border-radius: 0 0 4px 4px; border-top: none; max-height: 250px; overflow-y: auto; box-shadow: 0 4px 6px rgba(0,0,0,0.1); z-index: 1000;">
              </div>
            </div>
          </div>
  ...
  <script>
    (function(){
      ...
      function updateSupplierDropdown(currentSupplierId) {
        ...
        // If a specific supplier is selected, update the search field
        if (currentSupplierId) {
          currentMarkers.forEach(function(marker) {
            if (marker.supplierData && marker.supplierData.id == currentSupplierId) {
              searchInput.value = marker.supplierData.name;
            }
          });
        }
      }
      
      // Function to show supplier list dropdown - make it global
      window.showSupplierList = function() {
        var listContainer = document.getElementById('supplierList');
        var searchInput = document.getElementById('supplierSearch');
        if (!listContainer || !searchInput) return;
        
        console.log('Showing supplier list, currentMarkers:', currentMarkers.length);
        populateSupplierList();
        listContainer.style.display = 'block';
        
        // Update search input styling to connect with dropdown
        searchInput.style.borderRadius = '4px 4px 0 0';
        searchInput.style.borderBottom = '1px solid #e2e8f0';
      };
      
      // Function to hide supplier list
      function hideSupplierList() {
        ...
        if (listContainer && searchInput) {
          setTimeout(function() {
            ...
            searchInput.style.borderBottom = '1px solid #cbd5e1';
          }, 200); // Delay to allow click to register
        }
      }
      
      // Function to populate the supplier list
      function populateSupplierList(filterText) {
        var listContainer = document.getElementById('supplierList');
        if (!listContainer) return;
        
        console.log('Populating list, filterText:', filterText, 'currentMarkers:', currentMarkers.length);
        listContainer.innerHTML = '';
        
        var filter = (filterText || '').toLowerCase();
        var activeFilters = getActiveFilters();
        var filteredMarkers = currentMarkers.filter(function(marker) {
          if (!marker.supplierData) return false;
          // Apply text filter from search bar
          var textMatch = true;
          if (filter) {
            textMatch = marker.supplierData.name.toLowerCase().includes(filter) ||
              (marker.supplierData.material && marker.supplierData.material.toLowerCase().includes(filter)) ||
              (marker.supplierData.city && marker.supplierData.city.toLowerCase().includes(filter));
          }
          return textMatch && markerMatchesFilters(marker.supplierData, activeFilters);
        });
        
        console.log('Filtered markers:', filteredMarkers.length);
        
        if (filteredMarkers.length === 0) {
          listContainer.innerHTML = '<div style="padding: 8px 12px; color: #94a3b8; font-size: 11px; font-style: italic;">No suppliers found</div>';
          return;
        }
        
        filteredMarkers.forEach(function(marker) {
          var item = document.createElement('div');
          item.style.cssText = 'padding: 8px 12px; cursor: pointer; font-size: 11px; color: #334155; border-bottom: 1px solid #f1f5f9;';
          item.textContent = marker.supplierData.name;
          item.onmouseover = function() { this.style.background = '#f8fafc'; };
          item.onmouseout = function() { this.style.background = '#fff'; };
          item.onclick = function() {
            selectSupplier(marker.supplierData);
          };
          listContainer.appendChild(item);
        });
      }
      
      // Function to filter supplier list based on search input - make it global
      window.filterSupplierList = function() {
        var searchInput = document.getElementById('supplierSearch');
        if (!searchInput) return;
        
        populateSupplierList(searchInput.value);
      };
      
      // Function to select a supplier
      function selectSupplier(supplierData) {
        var searchInput = document.getElementById('supplierSearch');
        if (searchInput) {
          searchInput.value = supplierData.name;
        }
        ...
        currentMarkers.forEach(function(marker) {
          if (marker.supplierData && marker.supplierData.id == supplierData.id) {
      ...
      function selectSupplierFromDropdown() {
        var dropdown = document.getElementById('supplierDropdown');
        if (!dropdown || !dropdown.value) return;
        
        var selectedId = parseInt(dropdown.value);
        
        // Find the supplier from current markers
        currentMarkers.forEach(function(marker) {
          if (marker.supplierData && marker.supplierData.id == selectedId) {
      ...
      // Close supplier list when clicking outside
      ...
      // Function to geocode address and plot marker
      function geocodeAndPlotMarker(supplier, popupContent) {
        // Build address string from supplier data
        var addressParts = [];
        ...
        if (!fullAddress) {
            // No address available, skip plotting this marker
            console.warn('No address for supplier: ' + supplier.name);
          return;
        }
        ...
        fetch(geocodeUrl, {
          headers: {
            'User-Agent': 'PortalSite/1.0' // Required by Nominatim
          }
        })
        ...
        .then(function(data) {
          if (data && data.length > 0) {
            ...
            plotMarker(supplier, popupContent, lat, lng);
          } else {
              // Geocoding failed, skip this marker
            console.warn('Geocoding failed for: ' + fullAddress);
          }
        })
        .catch(function(error) {
          console.error('Geocoding error:', error);
            // Skip this marker on error
        });
      }
