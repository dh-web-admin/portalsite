...
// Better stylesheet versioning for both localhost and production
$pageStyleFile = __DIR__ . '/style.css';
$pageStyleVersion = file_exists($pageStyleFile) ? filemtime($pageStyleFile) : time();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <link rel="stylesheet" href="style.css?v=<?php echo (int)$pageStyleVersion; ?>">
</head>
                    ...
                    <div class="ac-avatar-col">
                        ...
                        >
                            <?php if ($hasPhoto): ?>
                                <img
                                    ...
                                    alt="<?php echo htmlspecialchars($name); ?>'s photo"
                                    id="accAvatarImg"
                                    class="left-avatar-img left-avatar-plain"
                                >
                                <button
                                    type="button"
                                    id="accRemoveBtn"
                                    class="left-avatar-remove"
                                    title="Remove photo"
                                >✕</button>
                            <?php else: ?>
                                <div
                                    class="left-avatar-frame left-avatar-empty"
                                    id="accAvatarInitials"
                        ...
                        <div class="left-upload-wrap">
                            <button
                                type="button"
                                id="leftUploadBtn"
                                class="left-upload-btn"
                                onclick="accTriggerPicker()"
                            >
                                <?php echo $hasPhoto ? 'Change Photo' : 'Upload Photo'; ?>
                            </button>
                        </div>
                        ...
                        <div class="left-change-pw">
                            <button
                                type="button"
                                class="left-change-toggle"
                                id="leftChangeToggle"
                                aria-expanded="false"
                                aria-controls="leftChangePanel"
                            >
                                Change Password
                            </button>
                            ...
                            <div id="leftChangePanel" class="left-change-pw-content" hidden>
                                <?php if (isset($_SESSION['password_success'])): ?>
                                    <div class="ac-status ac-status--success">
                                        <?php
                                    ...
                                    <div class="left-pw-field">
                                        ...
                                        <div class="pw-input-wrap">
                                            ...
                                            <button
                                                type="button"
                                                class="pw-toggle-btn"
                                                onclick="accTogglePw('current_password', this)"
                                    ...
                                    <div class="left-pw-field">
                                        ...
                                        <div class="pw-input-wrap">
                                            ...
                                            <button
                                                type="button"
                                                class="pw-toggle-btn"
                                                onclick="accTogglePw('new_password', this)"
                                                aria-label="Toggle password visibility"
                                            >Show</button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="left-pw-field">
                                        ...
                                        <div class="pw-input-wrap">
                                            ...
                                            <button
                                                type="button"
                                                class="pw-toggle-btn"
                                                onclick="accTogglePw('confirm_password', this)"
                                                aria-label="Toggle password visibility"
                                            >Show</button>
                                        </div>
                                    </div>
...
(function () {
    ...
    if (existingRemoveBtn) {
        existingRemoveBtn.addEventListener('click', accRemovePhoto);
    }

    window.accTriggerPicker = function () {
        if (photoInput) {
            photoInput.click();
        }
    };
    ...
    function showStatus(type, msg) {
        ...
        if (type === 'success') {
            statusEl.classList.add('ac-status--success');
        }
        if (type === 'error') {
    ...
    window.accSubmit = function (e) {
        ...
        if (photoInput.files && photoInput.files[0]) {
            fd.set('profile_picture', photoInput.files[0]);
        }

        fetch('../../api/save_user_details.php', {
            method: 'POST',
            body: fd,
    ...
    function accRemovePhoto(ev) {
        ...
        .then(text => {
            ...
            try {
                ...
                console.error('remove: invalid json', text);
            ...
            if (data && data.success) {
                ...
                const rem = document.getElementById('accRemoveBtn');
                if (rem) {
                    rem.remove();
                }
    ...
    (function () {
        ...
        function clientValidate(current, nw, confirmPw) {
        ...
        pwForm.addEventListener('submit', function (ev) {
            ...
            if (vErr) {
                showStatus('error', vErr);
                return;
            }

            const fd = new FormData(pwForm);

            fetch('../../api/change_own_password.php', {
                method: 'POST',
                body: fd,
                credentials: 'same-origin',
            ...
            .then(text => {
                ...
                if (data && data.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = '/PortalSite/auth/logout.php';
                    }, 2200);
                } else {
    ...
    ['first_name', 'last_name', 'email'].forEach(function (id) {
        ...
        f.addEventListener('paste', function (ev) {
            ev.preventDefault();
            showReadonlyPopup(f, 'This field cannot be edited here');
        });
    });
    ...
    (function () {
        try {
            showStatus('success', <?php echo json_encode($_SESSION['password_success']); ?>);
        } catch (e) {}
    ...
    (function () {
        try {
            showStatus('error', <?php echo json_encode($_SESSION['password_error']); ?>);
        } catch (e) {}
