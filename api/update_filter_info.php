...
$stmt = $conn->prepare("UPDATE filter_info SET filter_date=?, hours=?, part_number=?, make=? WHERE filter_id=?");
if (!$stmt) {
    echo json_encode(['success' => false, 'error' => 'Prepare failed: ' . $conn->error]);
    exit();
}
$stmt->bind_param('ssssi', $filter_date, $hours, $part_number, $make, $filter_id);
