...
$stmt = $conn->prepare(
    'UPDATE hydraulic_component_specs SET
...
$stmt->bind_param(
    ...
    $pressureRatingPsi, $burstPressurePsi, $manufacturer, $id
);

// mysqli's default error mode throws on a failed execute() — including a
// unique-constraint violation — rather than returning false, so a Number
// collision with another row has to be caught, not branched on.
try {
    $ok = $stmt->execute();
...
if ($ok) {
    ...
    echo json_encode([
        'success' => false,
        'message' => $dup ? 'A component with that Number already exists' : 'Failed to save changes',
    ]);
}
$stmt->close();
