<?php
/**
 * Database Migration: Add 'developer' role
 * 
 * Run this script ONCE after deploying to production to add the 'developer' role to the database.
 * After running successfully, you can delete this file for security.
 * 
 * URL: https://yoursite.com/PortalSite/migrations/add_developer_role.php
 */
...
<html>
<head>
    <title>Database Migration - Add Developer Role</title>
    <style>
...
<body>
    <h1>🔧 Database Migration: Add Developer Role</h1>
    ...
    $alreadyHasDeveloper = strpos($column['Type'], 'developer') !== false;
    
    if ($alreadyHasDeveloper) {
        echo '<div class="info"><strong>✓ Already Applied:</strong> The developer role already exists in the database. No action needed.</div>';
        echo '<pre>' . htmlspecialchars($column['Type']) . '</pre>';
    } else {
        echo '<div class="info"><strong>Status:</strong> Developer role not found. Applying migration...</div>';
        
        // Apply the migration
        $migrationSql = "ALTER TABLE users MODIFY COLUMN role ENUM('admin','projectmanager','estimator','accounting','superintendent','foreman','mechanic','operator','laborer','developer') NOT NULL";
        
        if ($conn->query($migrationSql)) {
            echo '<div class="success"><strong>✓ Success:</strong> Developer role has been added to the database!</div>';
    ...
    <ul>
        <li>Adds 'developer' as a valid role option in the users table</li>
        <li>Allows you to assign users the developer role from the admin panel</li>
        <li>Developer users will see "Developer Preview Dashboard" when logged in</li>
    </ul>
    ...
    <ol>
        ...
        <li>Test assigning the developer role to a user</li>
        <li><strong>Delete this migration file</strong> from the server: <code>/migrations/add_developer_role.php</code></li>
    </ol>
</body>
</html>
